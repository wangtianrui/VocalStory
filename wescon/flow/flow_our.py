...
class CausalMaskedDiffWithXvec(torch.nn.Module):
    def __init__(self,
                 ...
                 decoder_conf: Dict = {'in_channels': 240, 'spk_emb_dim': 80, 'n_spks': 1,
                                       'cfm_params': DictConfig({'sigma_min': 1e-06, 'solver': 'euler', 't_scheduler': 'cosine',
                                                                 'training_cfg_rate': 0.2, 'inference_cfg_rate': 0.7, 'reg_loss_type': 'l1'}),
                                       },
                 mel_feat_conf: Dict = {'n_fft': 1024, 'num_mels': 80, 'sampling_rate': 22050,
                 ...
                 decoder: torch.nn.Module = None,):
        super().__init__()
        decoder_conf["estimator"] = ConditionalDecoder(
    ...
    def forward(
        ...
        # concat text and prompt_text
        mask = (~make_pad_mask(token_len)).to(embedding.dtype).unsqueeze(-1).to(device)
        token = self.input_embedding(torch.clamp(token, min=0)) * mask
        ...
        feat = F.interpolate(feat.unsqueeze(dim=1), size=h.shape[1:], mode="nearest").squeeze(dim=1)
        # get conditions
        conds = torch.zeros(feat.shape, device=token.device, dtype=h.dtype)
        for i, j in enumerate(feat_len):
            if random.random() < 0.5:
                continue
            index = random.randint(0, int(0.3 * j))
        ...
        feat_len = feat_len + (feat.size(1) - max(feat_len))
        mask = (~make_pad_mask(feat_len)).to(embedding.dtype).to(device)
        loss, _ = self.decoder.compute_loss(
            feat.transpose(1, 2).contiguous(),
            mask.unsqueeze(1),
            h.transpose(1, 2).contiguous(),
