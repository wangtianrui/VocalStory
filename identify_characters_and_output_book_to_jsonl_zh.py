...
async def identify_character_gender_and_age_using_llm_and_assign_score(character_name, index, lines):
    ...
    try:
        ...
        system_prompt = """
        {no_think_token}
        你是一位擅长分析人物姓名并根据人物姓名和文本片段推断其性别和年龄的专家。请综合人物姓名和文本片段，为该人物分配合适的年龄和性别。
        如果该人物是男性，请将性别标记为 'male'；如果是女性，请标记为 'female'；如果性别中性或无法判断，请标记为 'unknown'。
        关于年龄判断：如果人物是儿童，请标记为 'child'；如果是成年人，请标记为 'adult'；如果是老年人，请标记为 'elderly'。
        只需返回性别和年龄作为输出，不要进行任何解释或表达疑问。
        输出请使用以下格式的字符串：
        Age: <age>
        ...
        # 用户提示，包含人物姓名和对话上下文
        user_prompt = f"""
        人物姓名/人物描述: {character_name}

        文本片段: {text_character_dialogues}
        """
        
        # Query the LLM to infer age and gender
        age_and_gender = ernie.inference(system_prompt, user_prompt)
...
async def identify_characters_and_output_book_to_jsonl(text: str, protagonist):
    """
    处理给定文本，识别角色，分配性别得分，并将结果输出为 JSONL 文件。

    此函数执行以下步骤：
    1. 清空用于存储带说话人属性的文本行的现有 JSONL 文件。
    2. 使用命名实体识别（NER）从文本中识别出人物角色。
    3. 使用语言模型（LLM）为每个角色分配性别和年龄得分。
    4. 将处理后的带说话人标注的文本输出到一个 JSONL 文件中。
    5. 将角色的性别和年龄得分保存到一个单独的 JSON 文件中。

    参数:
        text (str): 要处理的输入文本，通常是书籍或剧本。
        protagonist: 文本中的主角，用作说话人识别的参考。

    输出:
        - speaker_attributed_book.jsonl: 每行包含一位说话人及其对应对话或叙述内容的 JSONL 文件。
        - character_gender_map.json: 包含每个角色性别与年龄得分的 JSON 文件。
    """
...
async def process_book_and_identify_characters(book_name):
    ...
    protagonist = await find_book_protagonist(book_name, ernie, model_name)
